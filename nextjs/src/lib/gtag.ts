export const GA_MEASUREMENT_ID = "G-H9LZ9YK9DR";
...
export function trackCtaClick(pagePath: string, ctaText: string) {
  sendEvent({
    action: "cta_click",
    params: { page_path: pagePath, cta_text: ctaText },
  });
}
...
export function trackPhoneClick(pagePath: string, phoneNumber: string) {
  sendEvent({
    action: "phone_click",
    params: { page_path: pagePath, phone_number: phoneNumber },
  });
}
...
export function trackFormStart(pagePath: string, firstField: string) {
  sendEvent({
    action: "form_start",
    params: { page_path: pagePath, first_field: firstField },
  });
}
...
export function trackScrollDepth(pagePath: string, depthThreshold: number) {
  sendEvent({
    action: "scroll_depth",
    params: { page_path: pagePath, depth_threshold: depthThreshold },
  });
}
